...
class PayDunyaService {
    ...
    public function handleCallback($payload) {
        try {
            ...
            if ($result['status'] === "completed") {
                // Double vérification du montant payé
                if (!isset($result['total_amount']) || (float)$result['total_amount'] !== (float)$this->config['subscription']['amount']) {
                    error_log("ALERTE SÉCURITÉ: Tentative de paiement avec un montant incorrect : " . 
                             ($result['total_amount'] ?? 'non défini') . " au lieu de " . $this->config['subscription']['amount']);
                    throw new Exception("Montant payé incorrect ou manipulation détectée");
                }
                ...
                try {
                    ...
                    // Vérifier d'abord si c'est un renouvellement ou un nouvel abonnement
                    $check_stmt = $this->db->prepare("SELECT payment_status FROM subscriptions WHERE id = ?");
                    $check_stmt->bind_param("i", $subscription_id);
                    $check_stmt->execute();
                    $current_status = $check_stmt->get_result()->fetch_assoc()['payment_status'];
                    
                    $is_renewal = ($current_status === 'expired');
                    
                    if ($is_renewal) {
                        // RENOUVELLEMENT : Mettre à jour l'abonnement expiré
                        ...
                        $stmt = $this->db->prepare("
                            ...
                            SET payment_status = 'completed', 
                                ...
                                receipt_url = ?,
                                updated_at = NOW()
                            WHERE id = ? AND payment_status = 'expired'
                        ");
                        $stmt->bind_param("ssi", $new_expiry_date, $result['receipt_url'], $subscription_id);
                    } else {
                        // NOUVEL ABONNEMENT : Mettre à jour le statut pending
                        $stmt = $this->db->prepare("
                            UPDATE subscriptions 
                            SET payment_status = 'completed', 
                                payment_date = NOW(),
                    ...
                    if ($stmt->affected_rows === 0) {
                        throw new Exception("Abonnement non trouvé ou statut incorrect");
                    }
                    ...
                    if (!$is_renewal) {
                        // NOUVEL ABONNEMENT : Créer le compte admin
                        $admin_id = $this->generateAdminId($subscription['school_name']);
                        $password = $this->generatePassword();

                        // Hasher le mot de passe pour une meilleure sécurité
                        $hashed_password = md5($password); // Utilisation de md5 pour la compatibilité avec le système existant
                        
                        // Créer le compte admin dans la table users (structure réelle)
                        $stmt = $this->db->prepare("
                            INSERT INTO users (
                        ...
                        // Créer l'entrée correspondante dans la table admin
                        $stmt2 = $this->db->prepare("
                            INSERT INTO admin (
                                id, name, password, phone, email, dob, hiredate, address, sex
                            ) VALUES (?, ?, ?, ?, ?, CURDATE(), CURDATE(), ?, 'Homme')
                        ");
                        $admin_name = 'Admin ' . $subscription['school_name'];
                        $address = 'Adresse par défaut';
                        $stmt2->bind_param(
                            "ssssss", 
                            $admin_id, 
                            $admin_name,
                            $hashed_password, // Utiliser le mot de passe hashé également dans la table admin
                            $subscription['admin_phone'],
                            $subscription['admin_email'],
                            $address
                        );
                        
                        // Exécuter les deux requêtes d'insertion
                        $stmt->execute();
                        $stmt2->execute();
                        ...
                        // RENOUVELLEMENT : Envoyer un email de confirmation
                        $this->sendRenewalConfirmationEmail(
                            $subscription['admin_email'],
                            $subscription['school_name']
                        );
                    }
    ...
    /**
     * Envoie un email de confirmation de renouvellement
     */
        ...
        try {
            ...
            <body>
                <div class='container'>
                    ...
                    <div class='content'>
                        <p>Cher administrateur,</p>
                        <p>Votre abonnement pour <strong>{$school_name}</strong> a été renouvelé avec succès.</p>
                        <p>Votre compte est maintenant actif pour une nouvelle période d'un an.</p>
                        <p style='text-align: center; margin-top: 30px;'>
                            <a href='{$login_url}' class='button'>Accéder à mon espace</a>
                        </p>
                    </div>
            ...
            Votre abonnement pour {$school_name} a été renouvelé avec succès.
            Votre compte est maintenant actif pour une nouvelle période d'un an.
