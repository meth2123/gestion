...
function getSchoolMonths() {
    // Retourner les mois de l'année scolaire, d'octobre à juin (9 mois)
    // Organisés dans l'ordre chronologique de l'année scolaire
    return [
        // Premiers mois (année de début de l'année scolaire)
        '10' => 'Octobre',
        '11' => 'Novembre',
        '12' => 'Décembre',
...
function getCurrentSchoolYear() {
    ...
    $schoolYear = ($currentMonth >= 7) ? $currentYear : $currentYear - 1;
...
function getStudentPayments($studentId) {
    ...
    error_log("Étudiant trouvé: " . $student['name'] . " (ID: " . $_SESSION['parent_id'] . ")");
    
    // Récupérer les paiements pour l'année scolaire en cours
    // Cela inclut les paiements d'octobre à décembre de l'année de début
    // et ceux de janvier à juin de l'année suivante
    $currentSchoolYear = $schoolYear; // Année de début de l'année scolaire
    $nextYear = $currentSchoolYear + 1; // Année suivante
    ...
    $query = "SELECT p.*, 
              ...
              WHERE p.studentid = ? AND 
                    ((p.year = ? AND p.month >= 10 AND p.month <= 12) OR 
                     (p.year = ? AND p.month >= 1 AND p.month <= 6))
              ORDER BY 
                    CASE 
                        WHEN p.year = ? AND p.month >= 10 THEN 0 -- Premiers mois (année de début)
                        WHEN p.year = ? AND p.month <= 6 THEN 1  -- Mois de l'année suivante
                        ELSE 2 -- Autres mois (ne devrait pas arriver avec notre filtre)
                    END,
                    p.month ASC";
              
    error_log("Requête SQL: " . $query);
    error_log("Paramètres: studentId = " . $studentId . ", currentSchoolYear = " . $currentSchoolYear . ", nextYear = " . $nextYear);
    
    // Ajout des paramètres supplémentaires pour le tri CASE
    $payments = db_fetch_all($query, [$studentId, $currentSchoolYear, $nextYear, $currentSchoolYear, $nextYear], 'siiii');
...
function getUnpaidMonths($paidMonths) {
    ...
    // Afficher tous les mois de l'année scolaire, qu'ils soient payés ou non
    error_log("Affichage de tous les mois de l'année scolaire");
    ...
    // Trier les mois impayés par ordre chronologique de l'année scolaire (octobre à juin)
    // Créer un tableau temporaire pour le tri
    $sortedUnpaidMonths = [];
    
    // Ordre chronologique des mois dans l'année scolaire
    $monthOrder = ['10', '11', '12', '1', '2', '3', '4', '5', '6'];
    
    // Réorganiser les mois impayés selon l'ordre chronologique de l'année scolaire
    foreach ($monthOrder as $monthNum) {
        if (isset($unpaidMonths[$monthNum])) {
            $sortedUnpaidMonths[$monthNum] = $unpaidMonths[$monthNum];
        }
    }
    
    error_log("Mois impayés triés: " . implode(", ", $sortedUnpaidMonths));
    return $sortedUnpaidMonths;
}
