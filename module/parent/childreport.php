...
function getStudentAverage($studentId) {
    ...
    if ($check_table && $class_id) {
        ...
        $grades = db_fetch_all(
            "SELECT stc.*, c.name as course_name, t.name as teacher_name,
                    COALESCE(c.coefficient, 1) as coefficient_value,
                    CASE 
                        WHEN stc.grade_type = 'examen' THEN 'Examen'
                        WHEN stc.grade_type = 'devoir' THEN 'Devoir ' || stc.grade_number
                        ELSE 'Autre'
    ...
    foreach ($grades as $grade) {
        ...
        $coefficient = floatval($grade['coefficient_value']);
                            ...
                            <div class="flex justify-between items-center">
                                <div>
                                    ...
                                    <div class="flex items-center mt-1">
                                        ...
                                        <span class="ml-3 px-3 py-1 text-sm font-medium rounded-full <?php echo (strtoupper($child['sex']) === 'M' || strtoupper($child['sex']) === 'MALE' || strtoupper($child['sex']) === 'MASCULIN' || strtoupper($child['sex']) === 'H' || strtoupper($child['sex']) === 'HOMME' || $child['sex'] === '1') ? 'bg-blue-700 text-white' : 'bg-pink-400 text-white'; ?>">
                                            <?php echo (strtoupper($child['sex']) === 'M' || strtoupper($child['sex']) === 'MALE' || strtoupper($child['sex']) === 'MASCULIN' || strtoupper($child['sex']) === 'H' || strtoupper($child['sex']) === 'HOMME' || $child['sex'] === '1') ? 'Garçon' : 'Fille'; ?>
                                        </span>
                                    </div>
                                </div>
