...
// Utiliser la même approche que dans les autres pages pour filtrer les classes
...
-- Utiliser la version simplifiée du filtre pour éviter les erreurs de référence
AND " . get_course_modify_filter_sql($teacher_id, 'simple') . "
";
...
$courses = [];
if ($selected_class) {
    // Débug - Afficher l'ID de classe sélectionnée
    error_log("Classe sélectionnée: $selected_class");
    
    // Récupérer tous les enseignants assignés à cette classe
    $teachers_for_class = db_fetch_all(
        "SELECT DISTINCT teacherid FROM course WHERE classid = ?",
        [$selected_class],
        's'
    );
    
    $teacher_ids = [];
    if ($teachers_for_class) {
        foreach ($teachers_for_class as $t) {
            $teacher_ids[] = $t['teacherid'];
        }
    }
    
    // Vérifier si l'enseignant connecté est assigné à cette classe
    $is_teacher_assigned = in_array($teacher_id, $teacher_ids);
    
    // Si l'enseignant connecté est assigné à cette classe, montrer ses cours
    // Sinon, montrer les cours de tous les enseignants de cette classe
    if ($is_teacher_assigned) {
        // Récupérer les cours directement assignés à l'enseignant connecté
        ...
        $courses_from_direct = db_fetch_all(
            "SELECT DISTINCT id, name FROM course 
             WHERE classid = ?",
            [$selected_class],
            's'
        );
    ...
    // Récupérer les cours assignés via student_teacher_course
    $courses_from_stc = db_fetch_all(
        "SELECT DISTINCT c.id, c.name 
         ...
         AND (c.name != 'anglais' OR (c.name = 'anglais' AND ? = 'cp'))",
        [$selected_class, $teacher_id, $selected_class == 'CLS-CP-A-208' ? 'cp' : ''],
        'sss'
    );
...
if ($selected_class) {
    ...
    // Débug - Afficher les informations de la classe
    error_log("Détails de la classe sélectionnée: " . print_r($class_info, true));
    
    if ($class_info) {
        // Afficher l'ID et le nom de la classe pour le débogage
        $content .= '<div class="alert alert-info mb-3">
            <strong>Débogage:</strong> ID de classe: ' . htmlspecialchars($selected_class) . ', 
            Nom de classe dans la base: ' . htmlspecialchars($class_info['name']) . '
        </div>';
