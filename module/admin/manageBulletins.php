...
$query = "WITH all_student_grades AS (
    SELECT 
        s.id as student_id,
        ...
        -- Ajouter un indicateur si la note existe
        CASE WHEN stc.grade IS NOT NULL AND stc.grade > 0 THEN 1 ELSE 0 END as has_grade
    FROM class c
...
-- Filtrer les doublons d'examens en gardant ceux qui ont des notes
filtered_grades AS (
    SELECT * FROM (
        SELECT 
            *,
            ROW_NUMBER() OVER (
                PARTITION BY student_id, course_name, grade_type, grade_number
                ORDER BY has_grade DESC
            ) as row_num
        FROM all_student_grades
    ) ranked_grades
    WHERE row_num = 1 AND (grade IS NOT NULL AND grade > 0)
),
student_grades AS (
    SELECT 
        student_id,
...
if (empty($bulletins)) {
    ...
    foreach ($bulletins as $bulletin) {
        ...
        <tr>
            ...
            <td>
                <div class="' . (($bulletin['general_average'] ?? 0) >= 10 ? 'text-success' : 'text-danger') . ' fw-bold">
                    ' . number_format($bulletin['general_average'] ?? 0, 2) . '/20
                </div>
