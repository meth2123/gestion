...
$table_columns = [];
$columns_result = $link->query("SHOW COLUMNS FROM class_schedule");
if ($columns_result) {
...
$required_columns = [
    'day_of_week' => ["VARCHAR(20) DEFAULT 'Lundi'", 'slot_id'],
    'semester' => ["INT(11) DEFAULT 1", 'room'],
    'academic_year' => ["VARCHAR(20) DEFAULT '2024-2025'", 'semester'],
    'room' => ["VARCHAR(50) DEFAULT ''", 'day_of_week']
];

$columns_added = false;

foreach ($required_columns as $column => $details) {
    $column_exists = isset($table_columns[$column]);
    
    if (!$column_exists) {
        $type = $details[0];
        $after = $details[1];
        $after_clause = isset($table_columns[$after]) ? "AFTER $after" : "";
        
        $alter_query = "ALTER TABLE class_schedule ADD COLUMN $column $type $after_clause";
        $link->query($alter_query);
        $columns_added = true;
        
        // Journaliser l'ajout de colonne
        error_log("Ajout de la colonne $column à la table class_schedule: $alter_query");
    }
}

// Si des colonnes ont été ajoutées, vérifier à nouveau la structure
if ($columns_added) {
    $table_columns = [];
    $columns_result = $link->query("SHOW COLUMNS FROM class_schedule");
    if ($columns_result) {
...
$day_of_week_exists = isset($table_columns['day_of_week']);
$semester_exists = isset($table_columns['semester']);
$academic_year_exists = isset($table_columns['academic_year']);
$room_exists = isset($table_columns['room']);

// Construire la requête de base en fonction des colonnes existantes
$base_query = "SELECT DISTINCT cs.id, cs.class_id, cs.subject_id, cs.teacher_id, cs.slot_id";

// Ajouter les colonnes en fonction de leur existence
if ($day_of_week_exists) {
    $base_query .= ", cs.day_of_week";
} else {
    $base_query .= ", 'Lundi' as day_of_week";
}
...
if ($semester_exists) {
    $base_query .= ", cs.semester";
} else {
    $base_query .= ", 1 as semester";
}

if ($academic_year_exists) {
    $base_query .= ", cs.academic_year";
} else {
    $base_query .= ", '2024-2025' as academic_year";
}

// Ajouter le reste de la requête qui est commun à tous les cas
$base_query .= ", cs.created_by, c.name as class_name, s.name as subject_name, t.name as teacher_name, 
    ts.start_time, ts.end_time, CONCAT(ts.start_time, ' - ', ts.end_time) as time_slot
    FROM class_schedule cs
    JOIN class c ON cs.class_id = c.id
    JOIN course s ON cs.subject_id = s.id
...
// Ajouter les colonnes de tri en fonction de leur existence
if ($day_of_week_exists) {
    $order_by .= "cs.day_of_week, ";
}

// Toujours trier par heure de début et nom de classe
$order_by .= "ts.start_time, c.name";
