...
$semester_filter = isset($_GET['semester_filter']) ? $_GET['semester_filter'] : '1';
...
// Ajouter le filtre de semestre
$chart_data_sql .= " AND stc.semester = ?";
...
// Ajouter le paramètre pour le semestre
$chart_params[] = $semester_filter;
$chart_types .= 's';
...
if ($grade_filter < 20) {
    $sql .= " AND stc.grade IS NOT NULL AND stc.grade <= ?";
}
...
if ($class_filter !== null) {
    $students_sql = "SELECT 
        ...
        COUNT(CASE WHEN stc.grade IS NOT NULL AND stc.grade < ? THEN 1 ELSE NULL END) as low_grades_count
    FROM students s
    JOIN student_teacher_course stc ON CAST(s.id AS CHAR) = CAST(stc.student_id AS CHAR)
    WHERE CAST(stc.class_id AS CHAR) = ? AND stc.semester = ?
    GROUP BY s.id
    HAVING low_grades_count > 0
    ORDER BY avg_grade ASC";
    
    $students_stmt = $link->prepare($students_sql);
    $students_stmt->bind_param('iss', $grade_filter, $class_filter, $semester_filter);
                ...
                <form method="GET" class="row g-3">
                    ...
                    <div class="col-md-4">
                        ...
                        <select class="form-select" id="semester_filter" name="semester_filter">
                            <option value="1" <?php echo $semester_filter === '1' ? 'selected' : ''; ?>>Semestre 1</option>
                            <option value="2" <?php echo $semester_filter === '2' ? 'selected' : ''; ?>>Semestre 2</option>
                            <option value="3" <?php echo $semester_filter === '3' ? 'selected' : ''; ?>>Semestre 3</option>
                        </select>
                    </div>
                    <div class="col-12">
