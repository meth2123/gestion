...
$admin_name = $login_session; // Utilisation de $login_session défini dans main.php
        ...
        <div class="col-12">
            <div class="alert alert-info d-flex justify-content-between align-items-center">
                <div>
                    ...
                    <strong>Importation en masse</strong> - Téléversez un fichier Excel pour ajouter plusieurs étudiants à la fois
                </div>
                <a href="importStudents.php" class="btn btn-success">
...
if($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['studentId'])){
    ...
    try {
        ...
        error_log("Mot de passe hashé généré pour l'étudiant $stuId");
    ...
    $stuDOB = $_POST['studentDOB'];
        ...
        try {
            // Insertion dans la table users avec un préfixe pour identifier clairement les élèves
            // Vérifier si l'ID étudiant existe déjà dans la table users
            $check_user_sql = "SELECT userid FROM users WHERE userid = ?";
            $check_user_stmt = $link->prepare($check_user_sql);
            if (!$check_user_stmt) {
                error_log("ERREUR préparation requête vérification users: " . $link->error);
                throw new Exception("Erreur lors de la vérification de l'utilisateur");
            }
            
            $check_user_stmt->bind_param("s", $stuId);
            $check_user_stmt->execute();
            $check_user_result = $check_user_stmt->get_result();
            
            if ($check_user_result->num_rows > 0) {
                error_log("ATTENTION: L'ID utilisateur $stuId existe déjà dans la table users");
                // On continue car c'est peut-être une mise à jour
            } else {
                // Insertion dans la table users - IMPORTANT: stocker le mot de passe en clair pour l'authentification
                // Cela est nécessaire car le système d'authentification vérifie le mot de passe en clair
                ...
                // Utiliser le mot de passe en clair pour la table users
                $stmt->bind_param("ss", $stuId, $plainPassword);
                if (!$stmt->execute()) {
                    error_log("ERREUR insertion users: " . $stmt->error . " pour l'ID: " . $stuId);
                    throw new Exception("Erreur lors de l'insertion dans la table users. Vérifiez que l'ID étudiant est unique et respecte le format attendu.");
                }
                error_log("Insertion dans users réussie pour l'ID: " . $stuId);
            }
