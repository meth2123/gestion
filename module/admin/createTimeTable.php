...
// Récupérer les matières (cours) uniquement pour les classes et enseignants assignés par cet administrateur
$subjects_query = "SELECT c.id, c.name, c.teacherid, c.classid 
                  FROM course c 
...
>>>>>>> c:\Users\DELL\.windsurf\worktrees\gestion\gestion-a995ea30\module\admin\createTimeTable.php
        if (!$valid_assignment) {
            ...
            $stmt = $link->prepare("
                ...
                WHERE class_id = ? AND subject_id = ? AND teacher_id = ? AND slot_id = ? AND day_of_week = ? AND semester = ? AND academic_year = ? AND CONVERT(created_by USING utf8mb4) = CONVERT(? USING utf8mb4)
            ");
            $stmt->bind_param("sssissss", $class_id, $subject_id, $teacher_id, $slot_id, $day_of_week, $semester, $academic_year, $admin_id);
            $stmt->execute();
            $result = $stmt->get_result();
            ...
            if ($duplicate_entry) {
                ...
                // Vérifier les conflits pour la classe (même classe, même créneau, même jour) - uniquement pour cet administrateur
                $stmt = $link->prepare("
                    SELECT COUNT(*) as count FROM class_schedule 
                ...
                ");
                $stmt->bind_param("ssisss", $class_id, $slot_id, $day_of_week, $semester, $academic_year, $admin_id);
                $stmt->execute();
                $result = $stmt->get_result();
                $class_conflict = $result->fetch_assoc()['count'] > 0;
                
                // Vérifier les conflits pour l'enseignant (même enseignant, même créneau, même jour) - uniquement pour cet administrateur
                $stmt = $link->prepare("
                    SELECT COUNT(*) as count FROM class_schedule 
                    WHERE teacher_id = ? AND slot_id = ? AND day_of_week = ? AND semester = ? AND academic_year = ? 
                    AND CONVERT(created_by USING utf8mb4) = CONVERT(? USING utf8mb4)
                ");
                $stmt->bind_param("ssisss", $teacher_id, $slot_id, $day_of_week, $semester, $academic_year, $admin_id);
                $stmt->execute();
                $result = $stmt->get_result();
