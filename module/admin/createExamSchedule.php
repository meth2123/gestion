...
if(!empty($_POST['submit'])){
    ...
    $classId = $_POST['classId']; // Ajout de la classe
    ...
    if($result->num_rows === 0) {
        ...
        $stmt = $link->prepare("INSERT INTO examschedule (id, examdate, time, courseid, classid, created_by) VALUES (?, ?, ?, ?, ?, ?)");
        $stmt->bind_param("ssssss", $id, $examDate, $examTime, $courseId, $classId, $created_by);
...
// Récupération des classes pour le select (uniquement celles créées par l'admin connecté)
$stmt = $link->prepare("SELECT id, name FROM class WHERE created_by = ? ORDER BY name");
$stmt->bind_param("s", $admin_id);
$stmt->execute();
$classes_result = $stmt->get_result();

// Vérifier s'il y a des cours et des classes disponibles
$has_courses = $courses_result->num_rows > 0;
$has_classes = $classes_result->num_rows > 0;
?>
            ...
            <div class="card-body">
                ...
                <div class="mb-3">
                    ...
                    <select name="classId" id="classId" required
                        class="form-select"
                        <?php echo !$has_classes ? 'disabled' : ''; ?>>
                        <option value="">Sélectionnez une classe</option>
                        <?php 
                        // Réinitialiser le pointeur de résultat pour les classes
                        if($has_classes) {
                    ...
                    <?php if(!$has_classes): ?>
                    <div class="form-text text-muted">Vous devez d'abord créer une classe avant de pouvoir planifier un examen.</div>
                    <?php endif; ?>
                </div>
            </div>
