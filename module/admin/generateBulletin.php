...
// Calcul de la moyenne générale pondérée sur toutes les notes (comme le web)
$general_total = 0;
$general_total_coef = 0;
foreach ($course_averages as $course => $data) {
    foreach ($data['grades'] as $grade) {
        $grade_value = $grade['grade'] ?? 0;
        $grade_coef = $grade['grade_coefficient'] ?? 1;
        $general_total += $grade_value * $grade_coef;
        $general_total_coef += $grade_coef;
    }
...
$general_average = $general_total_coef > 0 ? $general_total / $general_total_coef : 0;
...
$header = array('Matière', 'Enseignant', 'Devoir 1', 'Coef.', 'Devoir 2', 'Coef.', 'Examen', 'Coef.', 'Moyenne');
$w = array(40, 30, 18, 12, 18, 12, 18, 12, 25);
...
foreach ($course_averages as $course => $data) {
    // Initialiser les valeurs et les coefficients à afficher
    $devoir1 = $devoir1_coef = $devoir2 = $devoir2_coef = $examen = $examen_coef = '-';
    foreach ($data['grades'] as $grade) {
        if ($grade['grade_type'] === 'devoir') {
            if ($grade['grade_number'] == 1) {
                $devoir1 = isset($grade['grade']) ? number_format($grade['grade'], 2) . '/20' : '-';
                $devoir1_coef = isset($grade['grade_coefficient']) ? number_format($grade['grade_coefficient'], 2) : '-';
            } else {
                $devoir2 = isset($grade['grade']) ? number_format($grade['grade'], 2) . '/20' : '-';
                $devoir2_coef = isset($grade['grade_coefficient']) ? number_format($grade['grade_coefficient'], 2) : '-';
            }
        } else {
            $examen = isset($grade['grade']) ? number_format($grade['grade'], 2) . '/20' : '-';
            $examen_coef = isset($grade['grade_coefficient']) ? number_format($grade['grade_coefficient'], 2) : '-';
        }
    }
    ...
    $pdf->Cell($w[3], 6, $devoir1_coef, 1, 0, 'C');
    $pdf->Cell($w[4], 6, $devoir2, 1, 0, 'C');
    $pdf->Cell($w[5], 6, $devoir2_coef, 1, 0, 'C');
    $pdf->Cell($w[6], 6, $examen, 1, 0, 'C');
    $pdf->Cell($w[7], 6, $examen_coef, 1, 0, 'C');
    $pdf->Cell($w[8], 6, isset($data['average']) ? number_format($data['average'], 2) . '/20' : '-', 1, 0, 'C');
    $pdf->Ln();
}
