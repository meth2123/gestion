        ...
        if (!$valid_assignment) {
            ...
            if ($duplicate_entry) {
                ...
                if ($teacher_id_type === 'int') {
                    ...
                    // Si teacher_id est une chaîne, utiliser tel quel
                    $stmt = $link->prepare("
                        SELECT COUNT(*) as count FROM class_schedule 
                        WHERE teacher_id = ? AND slot_id = ? AND day_of_week = ? AND semester = ? AND academic_year = ?
                    ");
                ...
                if ($class_conflict) {
                    ...
                    $error_message = "Cet enseignant (ID: $teacher_id) a déjà un cours programmé le $day_of_week sur ce créneau horaire (ID: $slot_id) pour ce semestre et cette année académique.";
                } else {
...
function loadSubjectsAndTeachers($link, $class_id, $admin_id) {
    ...
    if (!empty($class_id)) {
        ...
        $stmt = $link->prepare("
            ...
            WHERE c.classid = ? AND c.created_by = ? AND t.created_by = ?
            ORDER BY c.name
        ");
        $stmt->bind_param("sss", $class_id, $admin_id, $admin_id);
        $stmt->execute();
        $result = $stmt->get_result();
                    ...
                    <form method="POST" action="" id="timetableForm">
                        ...
                        <div class="mb-3">
                            ...
                            <select name="semester" id="semester" class="form-select" required>
                                ...
                                <option value="3" <?php echo (isset($_POST['semester']) && $_POST['semester'] == '3') ? 'selected' : ''; ?>>Semestre 3</option>
                            </select>
                        </div>
